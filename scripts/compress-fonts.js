	...
	try {
		...
		for (const fontConfig of fonts) {
			...
			for (const fontFile of fontConfig.files) {
				// Keep source fonts outside public/. Deploy only subsetted WOFF2 files.
				const fontSrc = path.join(__dirname, "../assets/fonts-source", fontFile);
		...
		if (errors.length > 0) {
			...
			// 列出实际存在的字体文件
			const fontDir = path.join(__dirname, "../assets/fonts-source");
