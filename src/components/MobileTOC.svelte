...
let tocDepth = 3;
let homePanelTitle = i18n(I18nKey.postList);
...
const generatePostList = () => {
	...
	homePanelTitle =
		listContainer?.getAttribute("data-mobile-list-title") ||
		i18n(I18nKey.postList);

	// 查找所有文章卡片
	const postCards = document.querySelectorAll("#post-list-container .card-base");
	const items: Array<{
		title: string;
		url: string;
		pinned?: boolean;
	...
	postCards.forEach((card) => {
		...
		const titleLink = card.querySelector<HTMLAnchorElement>(
			'a.post-card-title[href], a[href*="/posts/"].transition.group',
		);
		// 查找分类链接
		// 查找置顶图标
...
>
	<div class="flex items-center justify-between mb-4">
		<h3 class="text-lg font-bold text-[var(--primary)]">{isHomePage ? homePanelTitle : i18n(I18nKey.tableOfContents)}</h3>
		<button 
			on:click={togglePanel}
